...
final class DiagnosisRepository {
  DiagnosisRepository(this._diagnosisService, this._secureStorage);
  ...
  Future<IntegratedAnalysisResult> runIntegratedAnalysis({
    ...
    if (normalizedAccessToken == null || normalizedAccessToken.isEmpty) {
      throw const DiagnosisRepositoryException('로그인이 필요합니다.');
    }

    try {
      return await _diagnosisService.runIntegratedAnalysis(
        examId: examId,
      ...
      throw DiagnosisRepositoryException(error.message);
    } catch (_) {
      throw const DiagnosisRepositoryException('통합 AI 분석을 완료하지 못했습니다.');
    }
  }
}

final class DiagnosisRepositoryException implements Exception {
  const DiagnosisRepositoryException(this.message);
