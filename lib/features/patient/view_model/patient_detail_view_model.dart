...
final class PatientDetailViewModel extends ChangeNotifier {
  ...
  Future<void> loadPatientDetail(String patientId) async {
    ...
    if (normalizedPatientId.isEmpty) {
      _errorMessage = '환자 ID가 올바르지 않습니다.';
      notifyListeners();
      return;
    }
    ...
    try {
      ...
      _patientDetail = await _patientRepository.getPatientDetail(
        normalizedPatientId,
      );
    } catch (error) {
      _patientDetail = null;
  ...
  Future<void> _loadAccessToken() async {
    ...
    if (token == null || token.trim().isEmpty) {
      ...
      throw const PatientDetailViewModelException('로그인 정보가 없습니다. 다시 로그인해 주세요.');
    }
  ...
  String _cleanErrorMessage(Object error) {
    return error
        ...
        .replaceFirst('PatientDetailViewModelException: ', '')
        .replaceFirst('Exception: ', '')
        .trim();
  }
}

final class PatientDetailViewModelException implements Exception {
  const PatientDetailViewModelException(this.message);
